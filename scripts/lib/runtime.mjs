...
export function buildLocalRuntimeEnv(localAppUrl) {
	...
	const localEnv = {
		...
		DATABASE_URL: process.env.DATABASE_URL || `postgresql://${encodeSegment(postgresUser)}:${encodeSegment(postgresPassword)}@localhost:5432/${encodeSegment(postgresDatabase)}`,
